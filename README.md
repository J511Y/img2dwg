...
**최적화 옵션**:
- `--layout-analysis` / `--no-layout-analysis`: 🚀 **고수준 레이아웃 분석** (95~99% 절감, 40만 토큰 → 4~20k 토큰)
  - 수천 개의 LINE → 수십 개의 WALL/ROOM 객체로 변환
  - 의미론적 구조 파악 (벽, 방, 문, 창문 등)
  - 반복 패턴 감지 및 템플릿화
- `--optimize` / `--no-optimize`: 기본 최적화 (좌표 반올림, 기본값 제거, DXF R2000)
- `--rdp-tolerance`: 폴리라인 간소화 허용 오차 (0.5=보수적, 1.0=권장, 2.0=공격적)
- `--compact-schema` / `--no-compact-schema`: Compact 스키마 사용 (키 단축, 배열 평탄화)
...
- `--model`: 토큰 계산에 사용할 모델 (기본: cl100k_base)
- `--split-ratio`: Train/Validation 분할 비율 (기본: 0.8)
- `--enable-tiling` / `--no-enable-tiling`: 토큰 초과 시 자동 타일링 on/off
- `--optimize` / `--no-optimize`: 데이터셋 생성 최적화 옵션 on/off

**이미지 URL 옵션** (권장):
- `--use-image-url` / `--no-use-image-url`: 이미지를 공개 URL로 사용 (기본값: 비활성화)
  - **효과**: 이미지 토큰 99% 절감 (300k → 15 토큰)
  - **비용**: 파인튜닝 비용 30배 절감
- `--image-service`: 업로드 서비스 선택 (기본: `github`)
  - `imgur`: 간단, 무료
  - `github`: 영구 보관
  - `cloudinary`: 이미지 최적화
