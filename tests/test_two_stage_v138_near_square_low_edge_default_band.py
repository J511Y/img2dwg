...
def test_two_stage_v138_near_square_low_edge_default_band_reinforces_low_edge_square_pocket(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_near_square_low_edge_default_band.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=170, height=136, contrast=0.745, edge_density=0.07),
    )
    ...
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=170, height=136, contrast=0.64, edge_density=0.14),
    )
