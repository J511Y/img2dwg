...
def test_consensus_v126_midedge_extension_gate_lifts_debias(monkeypatch, tmp_path: Path) -> None:
    image_path = tmp_path / "consensus_v126_midedge_extension.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=300, height=210, contrast=0.74, edge_density=0.36),
    )
    ...
    gate_off = strategy.run(
        ConversionInput(image_path=image_path, metadata={"consensus_score": 0.67}),
        tmp_path / "gate_off",
    )
