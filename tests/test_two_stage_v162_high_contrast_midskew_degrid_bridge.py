...
def test_two_stage_v162_high_contrast_midskew_degrid_bridge_boosts_debias(
    monkeypatch,
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "two_stage_v162_high_contrast_midskew_degrid_bridge.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Out-of-gate: complexity just below high-contrast bridge lower bound.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=158, height=100, contrast=0.71, edge_density=0.16),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # In-gate: midskew + high contrast + moderate edge density should trigger v162.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=158, height=100, contrast=0.72, edge_density=0.16),
    )
