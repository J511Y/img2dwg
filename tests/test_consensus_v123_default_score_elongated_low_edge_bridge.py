...
def test_consensus_v123_default_score_elongated_low_edge_bridge_lifts_tuning_when_gate_matches(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v123_default_score_elongated_low_edge.png"
    Image.new("RGB", (16, 16), color="white").save(image_path)

    strategy = ConsensusQAStrategy()

    # Gate-on sample: elongated + low-mid edge + default consensus score.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=302, height=220, contrast=0.70, edge_density=0.16),
    )
    ...
    # Gate-off control: same geometry/contrast, but edge density just below range.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=302, height=220, contrast=0.70, edge_density=0.12),
    )
