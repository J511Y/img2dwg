...
def test_two_stage_v94_mild_elongation_midband_relief_lifts_midband_corridor_layout(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "mild_elongation_midband_relief.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=420, height=300, contrast=0.44, edge_density=0.12),
    )
    ...
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=360, height=300, contrast=0.44, edge_density=0.12),
    )
