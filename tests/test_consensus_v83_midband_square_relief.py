...
def test_consensus_v83_midband_square_relief_lifts_mid_consensus_mild_skew_layouts(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v83_midband_square.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=336, height=260, contrast=0.66, edge_density=0.18),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=286, height=272, contrast=0.58, edge_density=0.10),
    )
