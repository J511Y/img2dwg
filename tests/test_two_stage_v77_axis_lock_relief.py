...
def test_two_stage_v77_axis_lock_relief_lifts_elongated_mid_complexity(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "axis_lock_relief.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Same complexity, but elongated geometry should trigger v77 axis-lock relief.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=510, height=270, contrast=0.61, edge_density=0.39),
    )
    ...
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=360, height=300, contrast=0.61, edge_density=0.39),
    )
