...
def test_consensus_v109_near_square_residual_degrid_relief_lifts_near_square_case(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v109_near_square_residual.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=256, height=245, contrast=0.52, edge_density=0.30),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=270, height=245, contrast=0.52, edge_density=0.30),
    )
