...
def test_consensus_v137_near_square_lowedge_bridge_gate_lifts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v137_near_square_lowedge_bridge.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=236, height=220, contrast=0.74, edge_density=0.24),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=236, height=220, contrast=0.74, edge_density=0.16),
    )
