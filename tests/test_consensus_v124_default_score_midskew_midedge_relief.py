...
def test_consensus_v124_default_score_midskew_midedge_relief_lifts_tuning_when_gate_matches(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v124_default_score_midskew_midedge.png"
    Image.new("RGB", (16, 16), color="white").save(image_path)

    strategy = ConsensusQAStrategy()

    # Gate-on sample: moderate skew + mid edge + default consensus score.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=288, height=210, contrast=0.73, edge_density=0.24),
    )
    ...
    # Gate-off control: same geometry/contrast, but edge density below range.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=288, height=210, contrast=0.73, edge_density=0.18),
    )
