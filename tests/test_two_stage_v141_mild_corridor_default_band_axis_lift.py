...
def test_two_stage_v141_mild_corridor_default_band_axis_lift_boosts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v141_mild_corridor_default_band_axis_lift.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: aspect ratio below v141 lower bound with same complexity/edge.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=106, height=100, contrast=0.58, edge_density=0.18),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: mild corridor + default-band complexity should trigger v141.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=132, height=100, contrast=0.58, edge_density=0.18),
    )
