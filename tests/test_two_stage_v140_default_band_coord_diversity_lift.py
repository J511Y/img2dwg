...
def test_two_stage_v140_default_band_coord_diversity_lift_boosts_debias(monkeypatch, tmp_path: Path) -> None:
    image_path = tmp_path / "two_stage_default_band_coord_diversity.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: same edge profile but complexity below v140 lower bound.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=176, height=128, contrast=0.30, edge_density=0.05),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: mild/moderate skew + mid/default complexity should trigger v140.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=176, height=128, contrast=0.60, edge_density=0.12),
    )
