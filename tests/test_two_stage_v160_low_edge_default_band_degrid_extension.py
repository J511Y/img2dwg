...
def test_two_stage_v160_low_edge_default_band_degrid_extension_boosts_debias(
    monkeypatch,
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "two_stage_v160_low_edge_default_band_degrid.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: edge density just below the new lower edge band.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=180, height=128, contrast=0.56, edge_density=0.07),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: mild/moderate skew + low-edge default complexity should trigger v160.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=180, height=128, contrast=0.56, edge_density=0.08),
    )
