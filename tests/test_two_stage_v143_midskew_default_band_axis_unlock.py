...
def test_two_stage_v143_midskew_default_band_axis_unlock_boosts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v143_midskew_default_band_axis_unlock.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: aspect ratio just below v143 lower bound while preserving
    # complexity score as in-gate (0.4*contrast + 0.6*edge = 0.34).
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=127, height=100, contrast=0.58, edge_density=0.18),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: mild mid-skew + default-band complexity should trigger v143.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=136, height=100, contrast=0.58, edge_density=0.18),
    )
