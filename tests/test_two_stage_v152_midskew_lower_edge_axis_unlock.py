...
def test_two_stage_v152_midskew_lower_edge_axis_unlock_boosts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v152_midskew_lower_edge_axis_unlock.png"
    ...
    # below the v152 lower bound so the bridge should stay off.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=130, height=100, contrast=0.56, edge_density=0.13),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: mild-to-mid skew + default-band complexity + lower-edge pocket.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=130, height=100, contrast=0.56, edge_density=0.18),
    )
