...
def test_two_stage_v76_orthogonal_relief_tail_lifts_skewed_mid_complexity(monkeypatch, tmp_path: Path) -> None:
    image_path = tmp_path / "orthogonal_relief.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Same complexity profile, but higher aspect should trigger v76 relief.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=520, height=260, contrast=0.74, edge_density=0.44),
    )
    ...
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=360, height=270, contrast=0.74, edge_density=0.44),
    )
