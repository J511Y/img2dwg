...
def test_two_stage_v130_near_square_midedge_default_fallback_reduces_axis_bias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "near_square_midedge_default.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=290, height=276, contrast=0.72, edge_density=0.22),
    )
    out_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_gate")

    # Move only edge density just below the gate floor to disable the lift.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=290, height=276, contrast=0.72, edge_density=0.13),
    )
