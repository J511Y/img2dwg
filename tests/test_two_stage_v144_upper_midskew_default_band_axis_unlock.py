...
def test_two_stage_v144_upper_midskew_default_band_axis_unlock_boosts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v144_upper_midskew_default_band_axis_unlock.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: aspect ratio just below v144 lower bound while preserving
    # default-band complexity score (0.4*contrast + 0.6*edge = 0.34).
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=147, height=100, contrast=0.58, edge_density=0.18),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # in-gate: upper mid-skew + default-band complexity should trigger v144.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=155, height=100, contrast=0.58, edge_density=0.18),
    )
