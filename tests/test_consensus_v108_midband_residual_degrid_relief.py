...
def test_consensus_v108_midband_residual_degrid_relief_lifts_moderate_band_cases(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v108_midband_residual.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=330, height=235, contrast=0.58, edge_density=0.30),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=280, height=250, contrast=0.54, edge_density=0.26),
    )
