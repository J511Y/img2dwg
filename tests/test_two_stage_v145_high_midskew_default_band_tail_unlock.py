...
def test_two_stage_v145_high_midskew_default_band_tail_unlock_boosts_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v145_high_midskew_default_band_tail_unlock.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # out-of-gate: aspect ratio just below v145 lower bound while preserving
    ...
    # in-gate: high mid-skew + default-band complexity should trigger v145.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=190, height=100, contrast=0.58, edge_density=0.18),
    )
