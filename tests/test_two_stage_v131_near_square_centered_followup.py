...
def test_two_stage_v131_near_square_centered_followup_reduces_axis_bias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "near_square_centered_followup.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=286, height=272, contrast=0.72, edge_density=0.24),
    )
    out_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_gate")

    # Move only edge density just below the gate floor to disable the follow-up lift.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=286, height=272, contrast=0.72, edge_density=0.17),
    )
