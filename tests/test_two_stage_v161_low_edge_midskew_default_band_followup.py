...
def test_two_stage_v161_low_edge_midskew_default_band_followup_boosts_debias(
    monkeypatch,
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "two_stage_v161_low_edge_midskew_default_band_followup.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Out-of-gate: edge density just below follow-up extension lower bound.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=188, height=128, contrast=0.56, edge_density=0.11),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # In-gate: low-edge midskew + default-band complexity should trigger v161.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=188, height=128, contrast=0.56, edge_density=0.12),
    )
