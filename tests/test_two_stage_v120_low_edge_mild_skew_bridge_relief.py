...
def test_two_stage_v120_low_edge_mild_skew_bridge_relief_improves_diversity(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "low_edge_mild_skew_bridge.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Gate-on: mild skew + mid/default complexity + low-mid edge density.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=312, height=240, contrast=0.60, edge_density=0.16),
    )
    out_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_gate")

    # Keep geometry/complexity nearby but move edge below gate floor.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=312, height=240, contrast=0.60, edge_density=0.10),
    )
