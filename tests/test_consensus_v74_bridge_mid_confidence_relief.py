...
def test_consensus_v74_bridge_mid_confidence_relief_boosts_bridge_band_layouts(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v74_bridge_mid_conf.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=350, height=220, contrast=0.62, edge_density=0.48),
    )
    out_bridge = strategy.run(
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=300, height=220, contrast=0.62, edge_density=0.48),
    )
