...
def test_two_stage_v152_midskew_default_band_dense_edge_diag_reduces_axis_bias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "two_stage_v152_midskew_default_band_dense_edge_diag.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Out-of-gate: edge density just below v152 lower bound.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=148, height=100, contrast=0.60, edge_density=0.21),
    )
    out_no_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_no_gate")

    # In-gate: midskew + default complexity + denser edge pocket.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=148, height=100, contrast=0.56, edge_density=0.27),
    )
