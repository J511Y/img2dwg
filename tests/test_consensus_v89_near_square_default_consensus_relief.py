...
def test_consensus_v89_near_square_default_consensus_relief_lifts_default_square_pocket(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v89_near_square_default.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=298, height=258, contrast=0.54, edge_density=0.17),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=266, height=252, contrast=0.54, edge_density=0.17),
    )
