...
def test_two_stage_v127_near_square_low_edge_residual_bridge_reduces_axis_bias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "near_square_low_edge_residual.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Gate-on pocket: near-square + low-edge + low-mid complexity.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=286, height=278, contrast=0.68, edge_density=0.14),
    )
    out_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_gate")

    # Keep everything similar but move edge slightly above the gate ceiling.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=286, height=278, contrast=0.59, edge_density=0.20),
    )
