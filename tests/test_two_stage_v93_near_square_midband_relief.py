...
def test_two_stage_v93_near_square_midband_relief_lifts_almost_square_mid_complexity_layout(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "near_square_midband_relief.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=324, height=300, contrast=0.62, edge_density=0.26),
    )
    ...
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=372, height=300, contrast=0.62, edge_density=0.26),
    )
