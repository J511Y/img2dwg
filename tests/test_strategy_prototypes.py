...
def test_two_stage_irrational_subpixel_segments_add_four_non_axis_segments() -> None:
    plan = SimpleNamespace(
        segments=[
            ((0.0, 0.0), (100.0, 0.0)),
            ((100.0, 0.0), (100.0, 100.0)),
    ...
    signals = SimpleNamespace(contrast=0.66, edge_density=0.52)

    appended = TwoStageBaselineStrategy._inject_irrational_subpixel_segments(plan, signals)
    ...
    rounded_x = {round(coord, 4) for start, end in injected for coord in (start[0], end[0])}
    rounded_y = {round(coord, 4) for start, end in injected for coord in (start[1], end[1])}
    assert len(rounded_x) >= 7
    assert len(rounded_y) >= 7
