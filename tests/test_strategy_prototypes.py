...
def test_two_stage_strategy_adds_anti_grid_diagonal_detail(tmp_path: Path) -> None:
    ...
    assert any("anti_grid_detail_diag:entropy_coordinate_lift_v41:" in note for note in out.notes)
...
def test_two_stage_irrational_phase_lattice_segments_lift_coordinate_diversity() -> None:
    plan = SimpleNamespace(
        segments=[
            ((0.0, 0.0), (100.0, 0.0)),
            ((100.0, 0.0), (100.0, 100.0)),
...
def test_two_stage_irrational_phase_lattice_plus6_segments_lift_coordinate_diversity() -> None:
    plan = SimpleNamespace(
        segments=[
            ((0.0, 0.0), (100.0, 0.0)),
            ((100.0, 0.0), (100.0, 100.0)),
    ...
    appended = TwoStageBaselineStrategy._inject_irrational_phase_lattice_plus6_segments(
        plan, signals
    )
    ...
    rounded_x = {round(coord, 4) for start, end in injected for coord in (start[0], end[0])}
    rounded_y = {round(coord, 4) for start, end in injected for coord in (start[1], end[1])}
    assert len(rounded_x) >= 12
    assert len(rounded_y) >= 12
