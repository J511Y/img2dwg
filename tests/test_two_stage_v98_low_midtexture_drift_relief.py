...
def test_two_stage_v98_low_midtexture_drift_relief_boosts_low_edge_midtexture_pocket(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "low_midtexture_drift_relief.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Relief-active pocket: mild elongation + low-mid complexity + low edges.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=410, height=300, contrast=0.40, edge_density=0.16),
    )
    out_relief = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_relief")

    # Control: matched geometry/complexity but edges above the v98 cap.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=410, height=300, contrast=0.205, edge_density=0.29),
    )
