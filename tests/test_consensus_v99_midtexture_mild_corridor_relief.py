...
def test_consensus_v99_midtexture_mild_corridor_relief_lifts_default_consensus_pocket(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v99_midtexture_mild_corridor.png"
    Image.new("RGB", (16, 16), color="white").save(image_path)

    strategy = ConsensusQAStrategy()

    # Relief-on pocket: mild elongation + low-mid texture + default consensus band.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=330, height=250, contrast=0.60, edge_density=0.14),
    )
    ...
    # Control: near-square shape and higher texture should not trigger v99 lift.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=286, height=270, contrast=0.70, edge_density=0.22),
    )
