...
def test_consensus_v78_axis_lock_proxy_relief_lifts_elongated_low_texture_layouts(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v78_axis_lock_proxy.png"
    ...
    monkeypatch.setattr(
        ...
        lambda _: ImageSignals(width=460, height=230, contrast=0.48, edge_density=0.33),
    )
    out_relief = strategy.run(
        ConversionInput(image_path=image_path, metadata={"consensus_score": 0.90}),
        tmp_path / "out_relief",
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=320, height=230, contrast=0.48, edge_density=0.33),
    )
