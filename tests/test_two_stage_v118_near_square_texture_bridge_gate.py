...
def test_two_stage_v118_near_square_texture_bridge_gate_reduces_axis_bias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "near_square_texture_bridge_gate.png"
    Image.new("RGB", (24, 24), color="white").save(image_path)

    strategy = TwoStageBaselineStrategy()

    # Gate-on sample: near-square + default-band complexity + slightly higher
    # edge density than v117 so the new bridge gate is the active lift.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=276, height=248, contrast=0.66, edge_density=0.25),
    )
    out_gate = strategy.run(ConversionInput(image_path=image_path), tmp_path / "out_gate")

    # Keep aspect/complexity in-band but move edge just above the gate ceiling
    # so the v118 bridge turns off.
    monkeypatch.setattr(
        two_stage,
        "extract_image_signals",
        lambda _: ImageSignals(width=276, height=248, contrast=0.585, edge_density=0.30),
    )
