...
def test_consensus_v128_near_square_low_edge_default_bridge_increases_debias(
    monkeypatch, tmp_path: Path
) -> None:
    image_path = tmp_path / "consensus_v128_near_square_low_edge_default_bridge.png"
    Image.new("RGB", (16, 16), color="white").save(image_path)

    strategy = ConsensusQAStrategy()

    # Gate-on pocket: near-square + low-edge + default-band consensus.
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=254, height=250, contrast=0.70, edge_density=0.16),
    )
    ...
    monkeypatch.setattr(
        consensus_qa,
        "extract_image_signals",
        lambda _: ImageSignals(width=254, height=250, contrast=0.70, edge_density=0.30),
    )
