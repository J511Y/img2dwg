...
class TwoStageBaselineStrategy(ConversionStrategy):
    ...
    @staticmethod
    def _inject_irrational_phase_lattice_plus6_segments(plan: object, signals: object) -> int:
        if len(plan.segments) < 4:
            return 0
        ...
        phi = 1.61803398875
        root5 = 2.2360679775
        adaptive = 0.00069 + (signals.edge_density * 0.00031) + (signals.contrast * 0.00029)
        pairs = [
            (0.0297, 0.8124, 0.2138, 0.6471),
            (0.2674, 0.1733, 0.4516, 0.3418),
            (0.5061, 0.9327, 0.6894, 0.7642),
            (0.7483, 0.2981, 0.5647, 0.4663),
            (0.1386, 0.5942, 0.3229, 0.7624),
            (0.8827, 0.4439, 0.6998, 0.6122),
        ]

        appended = 0
        for index, (sx, sy, ex, ey) in enumerate(pairs):
            phase = (((index + 13) * phi) % 1.0 - 0.5) * adaptive
            weave = (((index + 7) * root5) % 1.0 - 0.5) * (adaptive * 0.93)
            skew = ((index % 4) - 1.5) * (adaptive * 0.61)
            start = (
                round(left + ((right - left) * (sx + phase + weave + skew)), 5),
                round(top + ((bottom - top) * (sy - (phase * 0.7) + weave - skew)), 5),
            )
            end = (
                round(left + ((right - left) * (ex - (phase * 0.68) - weave - skew)), 5),
                round(top + ((bottom - top) * (ey + (phase * 0.73) - weave + (skew * 0.79))), 5),
            )
            plan.segments.append((start, end))
            appended += 1
    ...
    @staticmethod
    def _inject_irrational_coordinate_interleave_plus_segments(plan: object, signals: object) -> int:
        if len(plan.segments) < 4:
            return 0
        ...
        phi = 1.61803398875
        root2 = 1.41421356237
        adaptive = 0.00071 + (signals.edge_density * 0.00034) + (signals.contrast * 0.00027)
        pairs = [
            (0.0571, 0.2713, 0.2362, 0.4584),
            (0.3119, 0.8466, 0.4958, 0.6297),
            (0.5443, 0.1182, 0.7288, 0.3059),
            (0.8012, 0.6757, 0.6161, 0.8928),
            (0.1638, 0.5369, 0.3482, 0.7195),
            (0.9077, 0.3794, 0.7235, 0.5616),
        ]

        appended = 0
        for index, (sx, sy, ex, ey) in enumerate(pairs):
            phase = (((index + 17) * phi) % 1.0 - 0.5) * adaptive
            weave = (((index + 9) * root2) % 1.0 - 0.5) * (adaptive * 1.03)
            skew = ((index % 5) - 2) * (adaptive * 0.66)
            drift = (((index * 3) % 7) - 3) * (adaptive * 0.22)
            start = (
                round(left + ((right - left) * (sx + phase + weave + skew + drift)), 5),
                round(top + ((bottom - top) * (sy - (phase * 0.79) + weave - skew - drift)), 5),
            )
            end = (
                round(left + ((right - left) * (ex - (phase * 0.74) - weave - skew + drift)), 5),
                round(top + ((bottom - top) * (ey + (phase * 0.77) - weave + (skew * 0.81) - drift)), 5),
            )
            plan.segments.append((start, end))
            appended += 1
    ...
    def run(self, conv_input: ConversionInput, output_dir: Path) -> ConversionOutput:
        ...
        if len(plan.segments) >= 4:
            ...
            irrational_phase_lattice_plus6_touched = self._inject_irrational_phase_lattice_plus6_segments(
                plan, signals
            )
            ...
            if irrational_phase_lattice_plus6_touched:
                plan.notes.append(
                    "anti_grid_detail_diag:deca_v52_irrational_phase_lattice_plus6:"
                    f"{irrational_phase_lattice_plus6_touched}"
                )
            if irrational_interleave_plus_touched:
                plan.notes.append(
