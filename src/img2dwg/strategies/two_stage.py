...
class TwoStageBaselineStrategy(ConversionStrategy):
    ...
    @staticmethod
    def _inject_default_band_axis_escape_diag(
        plan: object,
        *,
        aspect_ratio: float,
        complexity: float,
        ...
        gain = 0.00029 + (complexity * 0.00019)
        start = (
            round(left + ((right - left) * (0.208 + (gain * 0.83))), 5),
            round(top + ((bottom - top) * (0.862 - (gain * 0.71))), 5),
        )
        end = (
            round(left + ((right - left) * (0.834 - (gain * 0.58))), 5),
            round(top + ((bottom - top) * (0.176 + (gain * 0.92))), 5),
        )
        plan.segments.append((start, end))
        return 1

    @staticmethod
    def _inject_default_band_axis_escape_bridge(
        plan: object,
        *,
        aspect_ratio: float,
        complexity: float,
        ...
        gain = 0.00027 + (complexity * 0.00018)
        start = (
            round(left + ((right - left) * (0.172 + (gain * 0.91))), 5),
            round(top + ((bottom - top) * (0.792 - (gain * 0.62))), 5),
        )
        end = (
            round(left + ((right - left) * (0.886 - (gain * 0.57))), 5),
            round(top + ((bottom - top) * (0.228 + (gain * 0.88))), 5),
        )
        plan.segments.append((start, end))
        return 1
    ...
    def run(self, conv_input: ConversionInput, output_dir: Path) -> ConversionOutput:
        ...
        axis_escape_diag_added = self._inject_default_band_axis_escape_diag(
            plan,
            aspect_ratio=aspect_ratio,
            complexity=complexity,
            edge_density=signals.edge_density,
        )
        if axis_escape_diag_added:
            plan.notes.append(
                f"anti_grid_detail_diag:pair_v156_default_band_axis_escape_diag:{axis_escape_diag_added}"
            )

        axis_escape_bridge_added = self._inject_default_band_axis_escape_bridge(
            plan,
            aspect_ratio=aspect_ratio,
            complexity=complexity,
            edge_density=signals.edge_density,
